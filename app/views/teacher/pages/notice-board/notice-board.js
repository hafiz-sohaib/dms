...
async function get_notices(query = "") {
    const response = await fetch((query !== "") ? `/api/v1/notices?search=${query}` : "/api/v1/notices");
    const responseData = await response.json();
    let output = "";

    if (responseData.notices.length > 0) {
...
// ==================== Search Notice ====================
document.getElementById('search_notice').addEventListener('keyup', function (event) {
